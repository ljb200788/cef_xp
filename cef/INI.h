...
fini_string_t& trimIni(fini_string_t& str, const fini_string_t trim_chars = "\t\v\f; ") {
...
/// Definition
INI::INI(const INI& from): source(from.source), file_name(from.file_name) {
  // Deep clone INI
...
INI::INI(fini_string_t file_name, bool doParse, int parseFlags): source(SOURCE_FILE), file_name(file_name) {
  this->create("");
...
bool INI::parse(int parseFlags) {
  ...
  {
    case SOURCE_FILE: {
      fini_ifstream_t file(file_name);
...
void INI::_parseFile(fini_ifstream_t& file, int parseFlags) {
  ...
  while(std::getline(file, line)) {
    ...
    if (trimIni(line).size() == 0) // Skip empty lines
      continue;

    if ('[' == line.at(0)) { // Section
      section = trimIni(line, "[] "); // Obtain key value, including contained spaced
...
bool INI::save(fini_string_t file_name, int saveFlags) {
  saveFlags = (saveFlags > 0)? saveFlags: SAVE_FLAGS;

  fini_ofstream_t file((file_name == "")? this->file_name : file_name, std::ios::trunc);
  if (!file.is_open())
     return false;
