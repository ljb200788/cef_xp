...
bool XMLUtil::ToBool( const char* str, bool* value )
{
    ...
    static const char* TRUEA[] = { "true", "True", "TRUE", 0 };
    static const char* FALSEA[] = { "false", "False", "FALSE", 0 };

    for (int i = 0; TRUEA[i]; ++i) {
        if (StringEqual(str, TRUEA[i])) {
            *value = true;
            return true;
        }
    }
    for (int i = 0; FALSEA[i]; ++i) {
        if (StringEqual(str, FALSEA[i])) {
            *value = false;
            return true;
        }
    }
